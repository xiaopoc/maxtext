...
def dense(
    ...
    Args:
        ...
        wgrad_precision: ``"fp8"`` for the standard quantized WGrad, or ``"bf16"``
            for rank-local BF16 operands with FP32 GEMM output and reduction.

    Returns:
        Transformed output tensor
    """
    if wgrad_precision not in ("fp8", "bf16"):
        raise ValueError(
            f"Unsupported Dense WGrad precision {wgrad_precision!r}; expected 'fp8' or 'bf16'."
        )
...
def _dense(
    ...
    Args:
        ...
        kernel_axes: Logical axes for sharding the weight matrix
        collective_op_set: A set of CollectiveOp objects for forward and backward passes.
        wgrad_precision: Operand precision policy for WGrad.
        quantizer_set: QuantizerSet which contains quantizers for different tensor types
...
def _dense_fwd_rule(
    ...
    wgrad_x = (
        with_sharding_constraint_by_logical_axes(x.astype(jnp.bfloat16), input_axes)
        if wgrad_precision == "bf16"
        else None
    )
    kernel_dtype_ref = (
        jnp.zeros((), dtype=kernel.dtype) if wgrad_precision == "bf16" else None
    )
    casted_x_lhs = (
        casted_x.get_tensor(usage=TensorUsage.LHS_TRANS).checkpoint(quantizer_set.x)
        if wgrad_precision == "fp8"
        else None
    )
    ctx = (
        casted_x_lhs,
...
def _dense_bwd_rule(
    ...
    fwd_x_contracting_dims, fwd_k_contracting_dims = map(
        tex.sanitize_dims, (len(x_shape), casted_kernel_rhs.ndim), contracting_dims
    )
